...
impl Window {
    ...
    /// Paint a custom Metal primitive into the scene for the next frame at the current z-index.
    ///
    /// The renderer is invoked inside GPUI's active command encoder and draws directly into the
    /// current drawable. GPUI applies the current content mask as a Metal scissor rectangle.
    #[cfg(target_os = "macos")]
    pub fn paint_metal(
        &mut self,
        bounds: Bounds<Pixels>,
        renderer: Arc<dyn crate::MetalPrimitiveRenderer>,
    ) {
        use crate::PaintMetal;

        self.invalidator.debug_assert_paint();

        let bounds = self.snap_bounds(bounds);
        let content_mask = self.snapped_content_mask();
        self.next_frame.scene.insert_primitive(PaintMetal {
            order: 0,
            bounds,
            content_mask,
            renderer,
