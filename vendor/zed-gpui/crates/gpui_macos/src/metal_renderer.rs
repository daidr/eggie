...
use gpui::{
    AtlasTextureId, Background, Bounds, ContentMask, DevicePixels, MetalRenderContext,
    MonochromeSprite, PaintMetal, PaintSurface, Path, Point, PolychromeSprite, PrimitiveBatch,
...
impl MetalRenderer {
    ...
    fn draw_primitives_to_texture(
        ...
        for batch in scene.batches() {
            let ok = match batch {
                ...
                PrimitiveBatch::Metal(range) => self.draw_custom_metal(
                    &scene.metal_primitives[range],
                    instance_buffer,
                    &mut instance_offset,
                    viewport_size,
                    command_encoder,
            ...
            if !ok {
                command_encoder.end_encoding();
                anyhow::bail!(
                    "scene too large: {} paths, {} shadows, {} quads, {} underlines, {} mono, {} poly, {} surfaces, {} custom metal",
                    scene.paths.len(),
                    scene.shadows.len(),
                    scene.quads.len(),
    ...
    fn draw_custom_metal(
        &self,
        primitives: &[PaintMetal],
        instance_buffer: &InstanceBuffer,
        instance_offset: &mut usize,
        viewport_size: Size<DevicePixels>,
        command_encoder: &metal::RenderCommandEncoderRef,
    ) -> bool {
        let viewport_width = i32::from(viewport_size.width).max(0) as f32;
        let viewport_height = i32::from(viewport_size.height).max(0) as f32;
        let full_scissor = MTLScissorRect {
            x: 0,
            y: 0,
            width: viewport_width as NSUInteger,
            height: viewport_height as NSUInteger,
        };

        for primitive in primitives {
            let visible_bounds = primitive.bounds.intersect(&primitive.content_mask.bounds);
            let min_x = visible_bounds
                .origin
                .x
                .as_f32()
                .floor()
                .clamp(0., viewport_width);
            let min_y = visible_bounds
                .origin
                .y
                .as_f32()
                .floor()
                .clamp(0., viewport_height);
            let max_x = (visible_bounds.origin.x + visible_bounds.size.width)
                .as_f32()
                .ceil()
                .clamp(min_x, viewport_width);
            let max_y = (visible_bounds.origin.y + visible_bounds.size.height)
                .as_f32()
                .ceil()
                .clamp(min_y, viewport_height);
            if max_x <= min_x || max_y <= min_y {
                continue;
            }
            command_encoder.set_scissor_rect(MTLScissorRect {
                x: min_x as NSUInteger,
                y: min_y as NSUInteger,
                width: (max_x - min_x) as NSUInteger,
                height: (max_y - min_y) as NSUInteger,
            });

            let mut context = MetalRenderContext::new(
                &self.device,
                command_encoder,
                viewport_size,
                primitive.bounds,
            ...
            primitive.renderer.paint(&mut context);
            if context.buffer_overflowed() {
                command_encoder.set_scissor_rect(full_scissor);
                return false;
            }
        }
