...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    fn advance_osc_string<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match byte {
            ...
            // 8-bit C1 String Terminator. The parser otherwise accepts the 7-bit ESC \\ form;
            // both terminate OSC payloads identically.
            0x9C if !self.osc_byte_continues_utf8() => {
                self.osc_end(performer, byte);
                self.state = State::Ground
            },
            0x18 | 0x1A => {
    ...
    /// Whether a raw C1-looking byte continues a UTF-8 scalar already buffered in the OSC.
    ///
    /// OSC payloads are byte-oriented, so U+672B (`E6 9C AB`) contains the same `9C` value as
    /// 8-bit ST. Only a standalone C1 byte terminates the string.
    fn osc_byte_continues_utf8(&self) -> bool {
        let mut prior_continuations = 0;
        for byte in self.osc_raw.iter().rev().take(3) {
            if (0x80..=0xBF).contains(byte) {
                prior_continuations += 1;
                continue;
            }
            let expected_continuations = match byte {
                0xC2..=0xDF => 1,
                0xE0..=0xEF => 2,
                0xF0..=0xF4 => 3,
                _ => return false,
            };
            return prior_continuations < expected_continuations;
        }
        false
    }
