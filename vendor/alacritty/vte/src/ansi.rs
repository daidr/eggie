...
pub trait Handler {
    ...
    /// OSC 9;4 terminal progress report.
    fn progress_report(&mut self, _: Option<ProgressReport>) {}

    /// OSC 7, OSC 9;9, or OSC 1337 CurrentDir working-directory report.
    fn report_working_directory(&mut self, _: String) {}

    /// OSC 133 semantic shell-integration marker.
    fn semantic_prompt(&mut self, _: SemanticPrompt) {}

    /// OSC 9, OSC 99, or OSC 777 desktop notification command.
    fn desktop_notification(&mut self, _: DesktopNotification) {}

    /// iTerm2 OSC 1337 extension not handled directly by another OSC branch.
    fn iterm2_command(&mut self, _: String, _: &str) {}

    /// Kitty rich clipboard protocol (OSC 5522).
    fn kitty_clipboard(&mut self, _: String, _: &str) {}

    /// Kitty file-transfer protocol (OSC 5113).
    fn kitty_file_transfer(&mut self, _: String, _: &str) {}
...
/// Visual state reported by OSC 9;4.
...
/// Terminal progress reported by OSC 9;4.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// OSC 133 semantic shell-integration action.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum SemanticPromptAction {
    ...
    PromptStart,
    InputStart,
    InputStartAndTerminatePrompt,
    OutputStart,
    CommandFinished,
}

/// OSC 133 marker with its unrecognized options preserved for forward compatibility.
...
/// Raw notification command. Parsing the OSC 99 metadata is intentionally deferred to the
/// terminal integration layer, which owns notification identity and response state.
#[derive(Clone, Debug, PartialEq, Eq)]
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Report the shell's current working directory as a file URI.
            b"7" if params.len() >= 2 => {
                self.handler.report_working_directory(joined(&params[1..]));
            },
            ...
            b"4" => {
                ...
                for chunk in params[1..].chunks(2) {
                    let (index, reported_index) = match chunk[0] {
                        b"-1" => (NamedColor::Foreground as usize, "-1".to_owned()),
                        b"-2" => (NamedColor::Background as usize, "-2".to_owned()),
                        index => match parse_number(index) {
                            Some(index) => (index as usize, index.to_string()),
                            None => {
                                unhandled(params);
                                continue;
                            },
                    ...
                    if let Some(c) = xparse_color(chunk[1]) {
                        ...
                        let prefix = alloc::format!("4;{reported_index}");
                        self.handler.dynamic_color_sequence(prefix, index, terminator);
                    } else {
                        unhandled(params);
                    }
                }
            ...
            // Report terminal task progress. This follows the OSC 9;4 contract used by
            // Windows Terminal, ConEmu, and compatible shell integrations.
            b"9" if params.get(1) == Some(&b"4".as_slice()) => {
                let parse_percent = |value: &[u8]| -> Option<u8> {
                    if value.is_empty() || value.iter().any(|byte| !byte.is_ascii_digit()) {
                        return None;
                    }

                    Some(value.iter().fold(0_u8, |percent, byte| {
                        percent
                            .saturating_mul(10)
                ...
                let report = match (params.get(2).copied(), params.get(3).copied(), params.len()) {
                    (Some(b"0"), None, 3) => Some(None),
                    (Some(b"1"), None, 3) => Some(Some(ProgressReport {
                        state: ProgressState::Normal,
                        percent: Some(0),
                    })),
                    (Some(b"1"), Some(percent), 4) => parse_percent(percent).map(|percent| {
                        Some(ProgressReport {
                            state: ProgressState::Normal,
                    ...
                    (Some(b"2"), None, 3) => {
                        Some(Some(ProgressReport { state: ProgressState::Error, percent: None }))
                    },
                    (Some(b"2"), Some(percent), 4) => parse_percent(percent).map(|percent| {
                        Some(ProgressReport { state: ProgressState::Error, percent: Some(percent) })
                    }),
                    (Some(b"3"), None, 3) => Some(Some(ProgressReport {
                        state: ProgressState::Indeterminate,
                        percent: None,
                    })),
                    (Some(b"4"), None, 3) => {
                        Some(Some(ProgressReport { state: ProgressState::Paused, percent: None }))
                    },
                    (Some(b"4"), Some(percent), 4) => parse_percent(percent).map(|percent| {
                        Some(ProgressReport {
                ...
                if let Some(report) = report {
                    self.handler.progress_report(report);
                } else {
                    unhandled(params);
                }
            },

            // ConEmu's current-working-directory extension.
            b"9" if params.get(1) == Some(&b"9".as_slice()) && params.len() >= 3 => {
                self.handler.report_working_directory(joined(&params[2..]));
            },

            // OSC 9 notifications use the remaining payload as their title.
            b"9" if params.len() >= 2 => {
                self.handler.desktop_notification(DesktopNotification {
                    code: 9,
                    payload: joined(&params[1..]),
                    terminator: terminator.to_owned(),
                });
            },

            // Kitty's notification protocol. Metadata and multipart payload state are handled by
            // the integration layer so it can also produce activation/close responses.
            b"99" if params.len() >= 2 => {
                self.handler.desktop_notification(DesktopNotification {
                    code: 99,
                    payload: joined(&params[1..]),
                    terminator: terminator.to_owned(),
            ...
            b"133" if params.len() >= 2 => {
                let action = match params[1] {
                    ...
                    b"A" => SemanticPromptAction::FreshLineAndPrompt,
                    b"N" => SemanticPromptAction::NewCommand,
                    b"P" => SemanticPromptAction::PromptStart,
                    b"B" => SemanticPromptAction::InputStart,
                    b"I" => SemanticPromptAction::InputStartAndTerminatePrompt,
                    b"C" => SemanticPromptAction::OutputStart,
                    b"D" => SemanticPromptAction::CommandFinished,
                    _ => return unhandled(params),
                };
                self.handler
                    .semantic_prompt(SemanticPrompt { action, options: joined(&params[2..]) });
            },

            // rxvt notification extension: `OSC 777;notify;title;body ST`.
            b"777" if params.len() >= 2 => {
                self.handler.desktop_notification(DesktopNotification {
                    code: 777,
                    payload: joined(&params[1..]),
                    terminator: terminator.to_owned(),
            ...
            // iTerm2 extensions. CurrentDir is normalized into the same event as OSC 7; all
            // other commands are preserved losslessly for the host integration.
            b"1337" if params.len() >= 2 => {
                let payload = joined(&params[1..]);
                if let Some(directory) = payload.strip_prefix("CurrentDir=") {
                    self.handler.report_working_directory(directory.to_owned());
                } else {
                    self.handler.iterm2_command(payload, terminator);
                }
            },

            // Kitty file-transfer protocol.
            b"5113" if params.len() >= 2 => {
                self.handler.kitty_file_transfer(joined(&params[1..]), terminator);
            },

            // Kitty rich clipboard protocol.
            b"5522" if params.len() >= 2 => {
                self.handler.kitty_clipboard(joined(&params[1..]), terminator);
            },
...
mod tests {
    ...
    struct MockHandler {
        ...
        progress_reports: Vec<Option<ProgressReport>>,
        titles: Vec<Option<String>>,
        working_directories: Vec<String>,
        semantic_prompts: Vec<SemanticPrompt>,
        notifications: Vec<DesktopNotification>,
        iterm2_commands: Vec<(String, String)>,
        kitty_clipboard_commands: Vec<(String, String)>,
        kitty_file_transfers: Vec<(String, String)>,
        hyperlinks: Vec<Option<Hyperlink>>,
        dynamic_color_queries: Vec<(String, usize, String)>,
    }
    ...
    impl Handler for MockHandler {
        ...
        fn progress_report(&mut self, report: Option<ProgressReport>) {
        ...
        fn semantic_prompt(&mut self, prompt: SemanticPrompt) {
            self.semantic_prompts.push(prompt);
        }

        fn desktop_notification(&mut self, notification: DesktopNotification) {
            self.notifications.push(notification);
        }

        fn iterm2_command(&mut self, payload: String, terminator: &str) {
            self.iterm2_commands.push((payload, terminator.to_owned()));
        }

        fn kitty_clipboard(&mut self, payload: String, terminator: &str) {
            self.kitty_clipboard_commands.push((payload, terminator.to_owned()));
        }

        fn kitty_file_transfer(&mut self, payload: String, terminator: &str) {
            self.kitty_file_transfers.push((payload, terminator.to_owned()));
        }
        ...
        fn dynamic_color_sequence(&mut self, prefix: String, index: usize, terminator: &str) {
            self.dynamic_color_queries.push((prefix, index, terminator.to_owned()));
        }
    }
    ...
    impl Default for MockHandler {
        fn default() -> MockHandler {
            MockHandler {
                ...
                iterm2_commands: Vec::new(),
                kitty_clipboard_commands: Vec::new(),
                kitty_file_transfers: Vec::new(),
                hyperlinks: Vec::new(),
    ...
    #[test]
    fn parse_osc_9_4_progress_with_all_terminators() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = MockHandler::default();

        parser.advance(&mut handler, b"\x1b]9;4;1;42\x07");
        parser.advance(&mut handler, b"\x1b]9;4;2;101\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;4;3\x9c");
        parser.advance(&mut handler, b"\x1b]9;4;4;7\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;4;0\x07");

        assert_eq!(
            handler.progress_reports,
            vec![
                Some(ProgressReport { state: ProgressState::Normal, percent: Some(42) }),
                Some(ProgressReport { state: ProgressState::Error, percent: Some(100) }),
                Some(ProgressReport { state: ProgressState::Indeterminate, percent: None }),
                Some(ProgressReport { state: ProgressState::Paused, percent: Some(7) }),
                None,
            ]
        );
    }
    ...
    #[test]
    fn parse_osc_9_4_defaults_and_ignores_malformed_reports() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = MockHandler::default();

        parser.advance(&mut handler, b"\x1b]9;4;1\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;4;2\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;4;4\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;4;9;50\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;4;1;abc\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;4;3;50\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;4;0;50\x1b\\");
        parser.advance(&mut handler, b"\x1b]9;5;1;50\x1b\\");

        assert_eq!(
            handler.progress_reports,
            vec![
                Some(ProgressReport { state: ProgressState::Normal, percent: Some(0) }),
                Some(ProgressReport { state: ProgressState::Error, percent: None }),
                Some(ProgressReport { state: ProgressState::Paused, percent: None }),
            ]
        );
    }

    #[test]
    fn parses_common_osc_integrations_losslessly() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = MockHandler::default();

        parser.advance(&mut handler, b"\x1b]1;tab title\x07");
        parser.advance(&mut handler, b"\x1b]7;file://user@example.test/tmp/a%20b\x1b\\");
        parser.advance(&mut handler, b"\x1b]8;id=docs;https://example.test/a;b\x1b\\");
        parser.advance(&mut handler, b"\x1b]8;;\x1b\\");
        parser.advance(&mut handler, b"\x1b]133;A;aid=42\x1b\\");
        parser.advance(&mut handler, b"\x1b]99;i=n1:p=body;hello\x1b\\");
        parser.advance(&mut handler, b"\x1b]777;notify;Build;Done\x07");
        parser.advance(&mut handler, b"\x1b]1337;RemoteHost=me@host\x1b\\");
        parser.advance(&mut handler, b"\x1b]5113;ac=send;id=t1\x1b\\");
        parser.advance(&mut handler, b"\x1b]5522;type=read;Lg==\x1b\\");

        assert_eq!(handler.titles, vec![Some("tab title".to_owned())]);
        assert_eq!(handler.working_directories, vec!["file://user@example.test/tmp/a%20b"]);
        assert_eq!(handler.hyperlinks.len(), 2);
        assert_eq!(handler.hyperlinks[0].as_ref().unwrap().id.as_deref(), Some("docs"));
        assert_eq!(handler.hyperlinks[0].as_ref().unwrap().uri, "https://example.test/a;b");
        assert!(handler.hyperlinks[1].is_none());
        assert_eq!(
            handler.semantic_prompts,
            vec![SemanticPrompt {
                action: SemanticPromptAction::FreshLineAndPrompt,
                options: "aid=42".to_owned(),
            }]
        );
        assert_eq!(handler.notifications[0].code, 99);
        assert_eq!(handler.notifications[0].payload, "i=n1:p=body;hello");
        assert_eq!(handler.notifications[0].terminator, "\x1b\\");
        assert_eq!(handler.notifications[1].code, 777);
        assert_eq!(handler.notifications[1].payload, "notify;Build;Done");
        assert_eq!(handler.notifications[1].terminator, "\x07");
        assert_eq!(
            handler.iterm2_commands,
            vec![("RemoteHost=me@host".to_owned(), "\x1b\\".to_owned())]
        );
        assert_eq!(
            handler.kitty_file_transfers,
            vec![("ac=send;id=t1".to_owned(), "\x1b\\".to_owned())]
        );
        assert_eq!(
            handler.kitty_clipboard_commands,
            vec![("type=read;Lg==".to_owned(), "\x1b\\".to_owned())]
        );
    }

    #[test]
    fn iterm2_negative_osc4_indices_query_default_colors() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = MockHandler::default();

        parser.advance(&mut handler, b"\x1b]4;-1;?;-2;?\x1b\\");

        assert_eq!(
            handler.dynamic_color_queries,
            vec![
                ("4;-1".to_owned(), NamedColor::Foreground as usize, "\x1b\\".to_owned(),),
                ("4;-2".to_owned(), NamedColor::Background as usize, "\x1b\\".to_owned(),),
            ]
        );
    }

    #[test]
    fn out_of_range_osc4_palette_indices_are_ignored() {
        let mut parser = Processor::<TestSyncHandler>::new();
        let mut handler = MockHandler::default();

        parser.advance(&mut handler, b"\x1b]4;999;#abcdef\x1b\\");
        parser.advance(&mut handler, b"\x1b]104;999\x1b\\");
