...
pub struct CellExtra {
    ...
    /// Ephemeral image-placement IDs carried through grid resize/reflow.
    ///
    /// These are installed immediately before a resize and removed immediately afterwards. They
    /// make an otherwise-empty anchor cell participate in reflow, mirroring a tracked grid pin
    /// without adding any persistent content to the terminal buffer.
    #[cfg_attr(feature = "serde", serde(skip))]
    reflow_anchors: Option<Box<ReflowAnchor>>,
}
...
impl Cell {
    ...
    pub(crate) fn push_reflow_anchor(&mut self, anchor: usize) {
        let extra = self.extra.get_or_insert(Default::default());
        let extra = Arc::make_mut(extra);
        extra.reflow_anchors = Some(Box::new(ReflowAnchor {
            index: anchor,
            next: extra.reflow_anchors.take(),
        }));
    }

    pub(crate) fn has_reflow_anchors(&self) -> bool {
        self.extra
            .as_ref()
            .is_some_and(|extra| extra.reflow_anchors.is_some())
    }

    pub(crate) fn take_reflow_anchors(&mut self) -> Vec<usize> {
        let Some(extra) = self.extra.as_mut() else {
            return Vec::new();
        };

        let (anchors, is_empty) = {
            let extra = Arc::make_mut(extra);
            let mut anchor = extra.reflow_anchors.take();
            let mut anchors = Vec::new();
            while let Some(current) = anchor {
                anchors.push(current.index);
                anchor = current.next;
            }
            let is_empty = extra.zerowidth.is_empty()
    ...
    fn is_empty(&self) -> bool {
        (self.c == ' ' || self.c == '\t')
            ...
            && self
                .extra
                .as_ref()
                .is_none_or(|extra| extra.zerowidth.is_empty() && extra.reflow_anchors.is_none())
    }
